...
public class RecurringPaymentRow
{
    ...
    public string ActionText => IsAlreadyBill ? "Already in bills" : "Add as bill";

    public string AmountRangeDisplay => MinAmount == MaxAmount
        ? Amount.ToString("C")
        : $"{MinAmount:C} - {MaxAmount:C}";

    public string AverageAmountDisplay => AverageAmount.ToString("C");
}
